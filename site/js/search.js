 ...
 */
var SearchRenderItem = function (title, url, content) {
    'use strict';
    var html = '';
    ...
    if ($(data).size() > 0) {
        ...
        $(data).each(function () {
            var title = this.titleNoFormatting,
                ...
                html = SearchRenderItem(title, url, content);
 ...
 */
var SearchQuery = function (query, start) {
    'use strict';
    ...
    var apiURL = 'http://ajax.googleapis.com/ajax/services/search/web?v=1.0&callback=?',
        requestData = {
            q   : 'site:catalogus-professorum.org ' + query,
            rsz : 8,
            start : start
        };
    ...
    $.ajax({
        ...
        success: function (data) {
            SearchRender(data.responseData.results);
